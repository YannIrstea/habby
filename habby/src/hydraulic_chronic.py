...
def chronic_hydro(merge_files, path_merges, discharge_input, discharge_output, name_prj, path_prj
    ...
    data at the discharge given in discharge_output. This function would only work well if the discharge in the outputs
    are close to the discharge present in the merge_files. Indeed, this function is just a linear interpolation, so
    it only functions for small changes in discharges. It cannot be used in cases where the discharge in discharge output
    is outside the range modelled or if there were not a sufficient number of discharges modelled.

    We assume that the substrate is not changing as a function of the discharge.
