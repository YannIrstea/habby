...
def check_hrr_description(list_reach, list_unit, fhrr_manager_file):
    '''
    Convert the user text file that describe the desired HRRs calculations into a list per reach
    of 2 unit number
    Args:
        list_reach: the ordered list of the reachs names eg [['Tronçon D'],['Tronçon K']]
        list_unit : the ordered list of units per reach  eg in case of discharge [['9.2', '35'],['9.2', '35', '78']]
        fhrr_manager_file: the path for a text file that describe what the user wants for calculating HRRs
            eg
            Unit1	Unit2	deltat[s]	reachname
            78	35	24017	Tronçon K
            35	9.2	2500	Tronçon K

            Note that in case of only one reach available in the hdf5 or only the first reach concern the reachname column is not necessary
            Note that only for the last column reachname the name of the reach may contain spaces
    Returns:
        listhrr: a list per reach of HRRs calculation defined by 3 informations [unit_number1, unit_number2, deltat]
            eg
            [[[2, 1, 24017.0],[1, 0, 2500.0]], []]

            note that unit_number1 must have a discharge/water_level/timestep > unit_number2 HRR only when dewatering !!
    Raises:
        listhrr:  a list [-99, errormessage] with error message a string that describe the problem
    '''
    filemantisse, file_extension = os.path.splitext(fhrr_manager_file)
    lchkcolhead = ['Unit1', 'Unit2', 'deltat[s]',
                   'reachname']  # see after possible no column 'reachname' in this case breachname= False and reachnumber=0
    nbcol = len(lchkcolhead)
    breachname = True
    listhrr = [[] for _ in range(len(list_reach))]
    with open(fhrr_manager_file, 'rt', encoding='utf-8') as ff:
        lines = ff.readlines()
            ...
            if iline == 0:
                if len(col) != nbcol:
                    if len(col) == nbcol - 1:
                        ...
                        errormessage = 'Error:  first line information ' + ' '.join(
                            lchkcolhead) + ' has not been  found correctly in ' + fhrr_manager_file + '.\n'
                        return [-99,errormessage]
                for i in range(nbcol):
                    if lchkcolhead[i].lower() != col[i].lower():
                        errormessage = 'Error:  first line information ' + lchkcolhead[
                ...
                if line.split() != '':
                    if len(col) < nbcol:
                        ...
                        return [-99,errormessage]
                    elif len(col) > nbcol:  # possible reach name with space eg "Tronçon D"
                        col2 = list(col[0:nbcol - 1])
                        tt = 0
                        for t in range(nbcol - 1):
                            tt = line.find(col[t], tt) + len(col[t])
                        col2.append(line[line.find(col[nbcol - 1], tt):])
                    else:
                        col2 = col
                    ...
                    if breachname :
                        ...
                        if numreach == -1:
                            errormessage = 'Error  line ' + str(
                                iline + 1) + '  reach name has not been found ' + fhrr_manager_file + '.\n'
                            return [-99,errormessage]
                    else:
                        numreach = 0
                    # check if Unit1 and Unit2 are known for the numreach get their unitnumber
                    l = []
                    for t in range(2):
                        for ilk, lk in enumerate(list_unit[numreach]):
                            if col2[t].lower() == lk.lower():
                                l.append(ilk)
                                break
                    if len(l) != 2:
                        errormessage = 'Error  line ' + str(
                            iline + 1) + ' at least one unitname has not been found ' + fhrr_manager_file + '.\n'
                        return [-99,errormessage]

                    # check if deltat is numeric and posisitive
                    if is_number(col2[2]):
                        l.append(float(col2[2]))
                    else:
                        errormessage='Error:  line ' + str(
                            iline + 1) + ' deltat is not numeric ' + fhrr_manager_file + '.\n'
                        return [-99,errormessage]
                    if float(col2[2]) <= 0:
                        errormessage='Error:  line ' + str(
                            iline + 1) + ' deltat is <=0 ' + fhrr_manager_file + '.\n'
                        return [-99,errormessage]
                    print(l)
                    listhrr[numreach].append(l)
        return listhrr
...
def hrr(hrr_description, progress_value, q=[], print_cmd=False, project_properties={}):
    ...
    for reach_number in reachnumberlist:
        ...
        hdf5_1.data_2d_whole[reach_number][0]["node"]["data"] = pd.DataFrame(
            hdf5_1.data_2d_whole[reach_number][0]["node"]["z"], columns=["z"])
        ...
        unit_counter_3 = -1
        for ophrr in listhrr[reach_number]:

            # progress
            delta_unit = delta_reach / len(range(len(hdf5_1.data_2d[0]) - 1, 0, -1))
            unit_counter_3 += 1

            unit_number1= ophrr[0]
            unit_number2 = ophrr[1]
            deltat = ophrr[2]
            q1 = hdf5_1.data_2d[reach_number][unit_number1].unit_name  # q1>q2
            q2 = hdf5_1.data_2d[reach_number][unit_number2].unit_name  # q2<q1
            # Todo (may be in check_hrr_description function) check that the discharge are increasing time step hydropeaking the flow is increasing or decreasing  TXT file must indicate time interval and the way the information is sorted
            tin1 = hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["tin"]
            tin2 = hdf5_1.data_2d[reach_number][unit_number2]["mesh"]["tin"]
            datamesh1 = hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["data"]  # TODO: pandas_array.iloc
            hdf5_1.data_2d[reach_number][unit_number1].c_mesh_mean_from_node_values('h')
            hmoy1 = hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["data"]['h'].to_numpy()
            hdf5_1.data_2d[reach_number][unit_number1].c_mesh_mean_from_node_values('z')
            zsurf1 = hmoy1 + hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["data"]['z'].to_numpy()
            hdf5_1.data_2d[reach_number][unit_number2].c_mesh_mean_from_node_values('h')
            hmoy2 = hdf5_1.data_2d[reach_number][unit_number2]["mesh"]["data"]['h'].to_numpy()
            hdf5_1.data_2d[reach_number][unit_number2].c_mesh_mean_from_node_values('z')
            zsurf2 = hmoy2 + hdf5_1.data_2d[reach_number][unit_number2]["mesh"]["data"]['z'].to_numpy()
            i_split1 = hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["data"]["i_split"]
            i_split2 = hdf5_1.data_2d[reach_number][unit_number2]["mesh"]["data"]["i_split"]
            xy1 = hdf5_1.data_2d[reach_number][unit_number1]["node"]["xy"]
            xy2 = hdf5_1.data_2d[reach_number][unit_number2]["node"]["xy"]
            h1 = hdf5_1.data_2d[reach_number][unit_number1]["node"]["data"]["h"].to_numpy()
            h2 = hdf5_1.data_2d[reach_number][unit_number2]["node"]["data"]["h"].to_numpy()
            z1 = hdf5_1.data_2d[reach_number][unit_number1]["node"]["data"]["z"].to_numpy()
            z2 = hdf5_1.data_2d[reach_number][unit_number2]["node"]["data"]["z"].to_numpy()
            # Todo ask Quentin
            area1 = 0.5 * (np.abs(
            ...
            datanode1 = hdf5_1.data_2d[reach_number][unit_number1]["node"]["data"].to_numpy()
            # datanode2 = hdf5_1.data_2d[reach_number][unit_number2]["node"]["data"].to_numpy()

            i_whole_profile1 = hdf5_1.data_2d[reach_number][unit_number1]["mesh"]["i_whole_profile"]
            i_whole_profile2 = hdf5_1.data_2d[reach_number][unit_number2]["mesh"]["i_whole_profile"]
                ...
                if len(anodelist) == 0:
                    ...
                    for l3 in anodelist:
                        if len(l3) == 1:
                            ...
                            datanode3.append(l3[2] * (datanode1[aixyzh[l3[1]]] - datanode1[aixyzh[l3[0]]]) + datanode1[
                                aixyzh[l3[0]]])
            ...
            def calculate_deltaz3(iwp, level1=4,
                                  minimum_surrounding_wetted_mesh=5):  # iwp, locawp, countcontactwp, sortwp1, sortwp2, rwp1, rwp2, tin1, tin2, zsurf1, zsurf2,level=4, minimum_surrounding_mesh=4
                        ...
                        if rwp2[iwp][1] == 0:  # CASE 1a & 1b the tin1 mesh has been dryed
                            ...
                            if i_split1[i11] == 0 and i_split2[i21] == 1:  # CASE 2a
                                ...
                                if not (bok):
                                    ...
                                    hrr_logfile += '[' + str(reach_number) + ',' + q1 + '-' + q2 + ']\t' + str(
                                        iwp) + '\tCASE_2a_1\n'
                                    iwholedone[iwp] = -1
                                    continue
                                else:
                            ...
                            if i_split1[i11] == 0 and i_split2[
                                i21] == 1:  # CASE 2b we assume that the second triangle of tin2 has also isplit=1
                        ...
                        if rwp2[iwp][1] == 0:  # CASE 1c the tin1 2 meshes has been dryed
                            ...
                            if i_split1[i11] == 1 and i_split2[i21] == 1:
                                ...
                                if rwp2[iwp][1] == 1:
                                    ...
                                    for k in range(5, 8):
                                        ...
                                        for ip in range(2):
                                            if (axyzh[p[ip][0], 0:2] == xyzh[k, 0:2]).all():
                                                ...
                                                bbok = False
                                                if d0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][2], 0:2],
                                                             xyzh[l67[0], 0:2]) < paramlimdist0:
                                                    if d0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][1], 0:2],
                                                                 xyzh[l67[1], 0:2]) < paramlimdist0:
                                                        affecta(7, l67[0])
                                                        affecta(6, l67[1])
                                                        bbok = True
                                                elif d0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][2], 0:2],
                                                               xyzh[l67[1], 0:2]) < paramlimdist0:
                                                    if d0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][1], 0:2],
                                                                 xyzh[l67[0], 0:2]) < paramlimdist0:
                                                        affecta(7, l67[1])
                                                        ...
                                                        bbok = True
                                                if bbok:
                                                    lambda7 = lambda0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][2], 0:2],
                                                                             axyzh[7, 0:2])
                                                    lambda6 = lambda0segment(axyzh[p[ip][0], 0:2], axyzh[p[ip][1], 0:2],
                                                                             axyzh[6, 0:2])
                                                    if (lambda7 > 0 and lambda7 < 1) and (lambda6 > 0 and lambda6 < 1):
                                                        bok = True
                                                        anodelist3 = [pp[ip], [p[ip][0], p[ip][2], lambda7],
                                                                      [p[ip][0], p[ip][1], lambda6]]
                                                        deltaz3_ = calculate_deltaz3(iwp)
                                    ...
                                    l56 = []
                                    for k in range(5, 9):
                                        if (axyzh[1, 0:2] == xyzh[k, 0:2]).all() or (
                                                axyzh[2, 0:2] == xyzh[k, 0:2]).all():
                                            l56.append(k)
                                    if len(l56) != 2:
                                        # Todo faire quelque chose
                                        hrr_logfile += '[' + str(reach_number) + ',' + q1 + '-' + q2 + ']\t' + str(
