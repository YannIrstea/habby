...
def hydrosignature_calculation(classhv, hyd_tin, hyd_xy_node, hyd_hv_node, hyd_data_node=None, iwholeprofile=None,
    ...
    if min_depth < np.min(cl_h) or max_depth > np.max(cl_h):
        print(
            "there is a problem in the class definition  fo h some hydraulic values are off the definition hydrosignature cannot be calculated")
        return
    if min_velocity < np.min(cl_v) or max_velocity > np.max(cl_v):
        print(
            "there is a problem in the class definition  fo v some hydraulic values are off the definition hydrosignature cannot be calculated")
        return
...
def hsexporttxt(pathexport, filename, classhv, unitname, nbmeshhs, total_area, total_volume, mean_depth, mean_velocity,
